...
// ContentHash: bf327a4a54b504caf4e82d80905dacf483f6ddacd4fc9cf188932f40e1e0a9af
// IndexVersion: 2
// --- END CODE INDEX META ---
...
namespace LagoVista.IoT.DataStreamConnector.Tests
{
    public class DataStreamConnectorTestBase
    {
        ...
        protected void AssertInvalidError(InvokeResult result, params string[] errs)
        {
            ...
            foreach (var err in errs)
            {
                Assert.IsTrue(result.Errors.Where(msg => msg.Message.Contains(err)).Any(), $"Could not find error [{err}]");
            }
        ...
        protected async Task ValidatePaginatedRecordSet(string deviceId, IDataStreamConnector connector)
        {
            ...
            getResult = await connector.GetItemsAsync(deviceId, new Core.Models.UIMetaData.ListRequest() { PageIndex = 2, PageSize = 15 });
            ...
            getResult = await connector.GetItemsAsync(deviceId, new Core.Models.UIMetaData.ListRequest() { PageIndex = 8, PageSize = 15 });
