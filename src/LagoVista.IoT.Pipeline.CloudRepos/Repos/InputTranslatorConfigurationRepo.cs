...
namespace LagoVista.IoT.Pipeline.CloudRepos.Repos
{
    public class InputTranslatorConfigurationRepo : DocumentDBRepoBase<InputTranslatorConfiguration>, IInputTranslatorConfigurationRepo
    {
        ...
        public InputTranslatorConfigurationRepo(IPipelineAdminRepoSettings settings, IAdminLogger logger) : base(settings.PipelineAdminDocDbStorage.Uri, settings.PipelineAdminDocDbStorage.AccessKey, settings.PipelineAdminDocDbStorage.ResourceName, logger)
        {
        ...
        public async Task<ListResponse<InputTranslatorConfigurationSummary>> GetInputTranslatorConfigurationsForOrgsAsync(string orgId, ListRequest listRequest)
        {
            ...
            return ListResponse<InputTranslatorConfigurationSummary>.Create(items.Model.Select(itm => itm.CreateSummary()), items);
        }
